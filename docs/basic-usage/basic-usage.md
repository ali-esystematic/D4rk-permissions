...
## Add The Trait
First, add the `D4rk\Permission\Traits\HasRoles` trait to your `User` model(s):
...
The `role` and `withoutRole` scopes can accept a string, a `\D4rk\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
The scope can accept a string, a `\D4rk\Permission\Models\Permission` object or an `\Illuminate\Support\Collection` object.
