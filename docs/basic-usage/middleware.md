...
```php
    ->withMiddleware(function (Middleware $middleware) {
        $middleware->alias([
            'role' => \D4rk\Permission\Middleware\RoleMiddleware::class,
            'permission' => \D4rk\Permission\Middleware\PermissionMiddleware::class,
            'role_or_permission' => \D4rk\Permission\Middleware\RoleOrPermissionMiddleware::class,
        ]);
    })
```
...
public static function middleware(): array
{
    return [
        ...
        new Middleware(\D4rk\Permission\Middleware\RoleMiddleware::using('manager'), except:['show']),
        new Middleware(\D4rk\Permission\Middleware\PermissionMiddleware::using('delete records,api'), only:['destroy']),
    ];
}
```
...
```php
Route::group(['middleware' => [\D4rk\Permission\Middleware\RoleMiddleware::using('manager')]], function () { ... });
Route::group(['middleware' => [\D4rk\Permission\Middleware\PermissionMiddleware::using('publish articles|edit articles')]], function () { ... });
Route::group(['middleware' => [\D4rk\Permission\Middleware\RoleOrPermissionMiddleware::using(['manager', 'edit articles'])]], function () { ... });
```
