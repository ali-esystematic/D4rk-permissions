...
The `assignRole`, `hasRole`, `hasAnyRole`, `hasAllRoles`, `hasExactRoles`  and `removeRole` functions can accept a
 string, a `\D4rk\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
string or a `D4rk\Permission\Models\Permission` object.
...
All these responses are collections of `D4rk\Permission\Models\Permission` objects.
